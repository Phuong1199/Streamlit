...
def load_models_and_data():
    with open(os.path.join(DATA_DIR, 'surprise_svd_model.pkl'), 'rb') as f:
        svd_algo = pickle.load(f)
    dictionary = corpora.Dictionary.load(os.path.join(DATA_DIR, 'gensim_dictionary.dict'))
    corpus = corpora.MmCorpus(os.path.join(DATA_DIR, 'gensim_corpus.mm'))
    tfidf = models.TfidfModel.load(os.path.join(DATA_DIR, 'gensim_tfidfmodel'))
    tfidf_matrix = scipy.sparse.load_npz(os.path.join(DATA_DIR, 'sklearn_tfidf_matrix.npz'))
    # Tải ma trận cosine similarity từ file .npz (sparse format)
    cosine_sim_sparse = scipy.sparse.load_npz(os.path.join(DATA_DIR, 'sklearn_cosine_sim_sparse.npz'))
    # Chuyển ma trận thưa thành ma trận mật (dense) để sử dụng
    cosine_sim = cosine_sim_sparse.toarray()
    df_product = pd.read_csv(os.path.join(DATA_DIR, 'processed_data.csv'))
    df_rating = pd.read_csv(os.path.join(DATA_DIR, 'processed_ratings.csv'))
